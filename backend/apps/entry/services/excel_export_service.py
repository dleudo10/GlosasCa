...
class ExcelExportService:
    ...
    def _generar_excel(
        ...
        """
        Genera el .xls final en formato binario BIFF (Excel 97-2003),
        que es lo que el software antiguo puede leer. Un .xlsx
        (aunque se le cambie la extensión a mano) no es compatible.

        Mantiene las dos hojas del exportador anterior:
        ...
        estilo_encabezado = xlwt.easyxf("font: bold on")

        hoja_facturas = workbook.add_sheet("FACTURAS_CG")
        self._escribir_hoja(
            hoja=hoja_facturas,
        ...
        hoja_detalles = workbook.add_sheet("DETALLES_CG")
        self._escribir_hoja(
            hoja=hoja_detalles,
    ...
    # ============================================================
    # ESCRITURA DE HOJA (tipos de dato + formato)
    # ============================================================
    ...
    def _escribir_hoja(
        ...
        estilo_encabezado: "xlwt.easyxf",
    ) -> None:
        """
        Escribe encabezado + filas respetando los tipos de dato
        que el software antiguo espera:
        ...
        for columna_idx, nombre_columna in enumerate(columnas):
            hoja.write(0, columna_idx, nombre_columna, estilo_encabezado)
        ...
        # Congelar encabezado (equivalente a freeze_panes = "A2")
        hoja.set_panes_frozen(True)
        hoja.set_horz_split_pos(1)
        hoja.set_remove_splits(True)

        # Ancho de columna aproximado según el contenido más largo
        for columna_idx, nombre_columna in enumerate(columnas):
            largo_max = len(nombre_columna)

            for fila in filas:
                valor = fila.get(nombre_columna, "")
                largo_max = max(largo_max, len(str(valor)))

            ancho = min(max(largo_max + 2, 12), 80)
            hoja.col(columna_idx).width = ancho * 256
