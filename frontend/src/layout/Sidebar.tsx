...
const othersItems: NavItem[] = [
    ...
    },
    {
        name: "Historial",
        // icon: <RotateCcwClock />,
        path: "/history-consents",
        permission: "consents.history"
    },
...
const Sidebar: React.FC = () => {
    ...
    const renderMenuItems = (items: NavItem[], menuType: "main" | "others") => (
        <ul className="flex flex-col gap-2">
            {items.filter(canShowItem).map((nav, index) => (
                <li key={nav.name}>
                    {nav.subItems ? (
                        <button
                    ...
                    {nav.subItems && (isExpanded || isHovered || isMobileOpen) && (
                        ...
                        >
                            <ul className="mt-2 space-y-1 ml-9">
                                {nav.subItems?.filter(item => !item.permission).map((subItem) => (
                                    <li key={subItem.name}>
                                        <Link
                                            to={subItem.path}
    ...
    const getVisibleItems = (items: NavItem[]) =>
        items.filter(canShowItem);

    const visibleNavItems = getVisibleItems(navItems);
    const visibleOthersItems = getVisibleItems(othersItems);
